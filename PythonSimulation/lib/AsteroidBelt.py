...
def createAstroidBeltFromActionAngleCoords(size, micro, J_1, J_2, J_3, M, w, Omega):
    a = np.square(J_1/micro)/micro
    ecc = np.sqrt(1 - np.square(J_2/J_1)) * u.one
    inc = np.arccos(J_3/J_2)
    return AsteroidBelt(size, micro, a, ecc, inc, Omega, w, M)


def generateNearSHODistributionNEWVERSION():
...
def generateNearSHODistribution(size, micro, m, r_0, epsilon):
    ...
    J_SHO = expon.rvs(scale=epsilon, loc=0, size=size)*u.kg*u.km**2/u.s

    #J_2 = expon.norm(scale=sigma_a, loc=r_0, size=size)
    J_2 = np.repeat(np.sqrt(r_0), size)*u.kg**0.5*u.km

    J_1 = (J_SHO + np.sqrt(np.power(J_SHO, 2) + 4 *
           micro*m*np.power(J_2, 2)))/(2.0*np.sqrt(micro*m))
    J_3 = J_2
    ...
    return createAstroidBeltFromActionAngleCoords(size, micro, J_1, J_2, J_3, M, w, Omega)
